...
def shuffle(x: "list[Any]", random: "Optional[Callable[[], float]]" = None) -> "None":
...
class TestLorem(unittest.TestCase):
    ...
    def test_gen_paragraph(self) -> "None":
        ...
        with self.mock_randint:
            paragraph = inst.gen_paragraph(
                comma=(0, 2), word_range=(1, 2), sentence_range=(2, 3)
            )
        ...
        with self.mock_randint:
            with self.mock_choice_first:
                paragraph = inst.gen_paragraph(
                    comma=(1, 2), word_range=(1, 2), sentence_range=(2, 3)
                )
        ...
        with self.mock_randint:
            with self.mock_choice_last:
                paragraph = inst.gen_paragraph(
                    comma=(1, 2), word_range=(2, 4), sentence_range=(3, 4)
                )
